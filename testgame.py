...
def print_menu(exits):
    ...
    for items in exits:
        print(print_menu_line(items, exit_leads_to(exits, items)))
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    if user_input in exits and exits[user_input] != "none":
        return True
    else:
        return False
...
def menu(exits):
    ...
    while True:
        ...
        #
        # Normalise the input
        normalise_input(player_inp)
        # Check if the input makes sense (is valid exit)
        is_valid = is_valid_exit(exits, player_inp)
